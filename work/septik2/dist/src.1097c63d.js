...
function toggleItem(item) {
  var isClosed = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : item.classList.contains('close');
...
accItem.forEach(function (item, index) {
  console.log(item);
  var header = item.querySelector('.accordionItemHeading');
  var content = item.querySelector('.accordionItemContent');
...
var openModal = function openModal() {
  ...
  setTimeout(function () {
    return document.addEventListener('click', handleOnModalClick);
  });
};
...
var handleOnModalClick = function handleOnModalClick(event) {
  var window = document.querySelector('[data-modal="window"]');
  var clickInside = window.contains(event.target);
...
Array.from(document.forms).forEach(function (form) {
  form.addEventListener("input", function (event) {
    var error = getErrorFormElement(event.target);
  ...
  form.addEventListener("submit", function (event) {
    ...
    var nameValue = form.elements.name;
    var error = getErrorFormElement(event.target);
    var invalid = !nameValue.value || !number.value || !email.value || !email.validity.valid;
